...
from pipeline._paths import GAMES_MARGIN, GAMES_SIM, IN_SEASON_PREFIX

from cfb_in_season_sim import actual_team_win, is_completed
...
def main() -> int:
    ...
    if out_path is None:
        if args.pin_completed or IN_SEASON_PREFIX in inp.stem:
            out_path = GAMES_SIM
        else:
            out_path = inp.with_name(inp.stem + "_simulated" + inp.suffix)
