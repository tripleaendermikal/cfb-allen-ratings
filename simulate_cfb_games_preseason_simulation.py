...
def _build_sim_fpi_lookup(
    rows: list[dict[str, str]], sim_cols: list[str]
) -> dict[str, dict[str, float]]:
    """team_id -> sim_col -> drawn FPI."""
    ...
    for row in rows:
        ...
        if not tid:
            continue
        for sc in sim_cols:
            if tid in out and sc in out[tid]:
                continue
            v = parse_float(row.get(sc))
            if v is not None:
                out[tid][sc] = v
    return out


def _simulate_pac12_flex(
    flex_rows_by_team: dict[str, dict[str, str]],
    margin_cols: list[str],
    win_cols: list[str],
    fpi_by_team: dict[str, dict[str, float]],
    rng: random.Random,
    *,
    main_seed: int | None = None,
    pin_completed: bool = False,
) -> dict[tuple[str, str], dict[str, str]]:
    """Simulate Week 13 Pac-12 flex games (bipartite home/away pairings per column)."""
    wins: dict[tuple[str, str], dict[str, str]] = {}
    for tid, row in flex_rows_by_team.items():
        wins[(row["game_id"], tid)] = {wc: "" for wc in win_cols}

    if not flex_rows_by_team:
        return wins

    for mc, wc in zip(margin_cols, win_cols):
        sim_col = margin_to_win_col(mc)
        sim_index = margin_col_index(mc)
        pairs = pair_home_away(pairing_rng(main_seed, sim_index))

        for home_id, away_id in pairs:
            home_row = flex_rows_by_team[home_id]
            away_row = flex_rows_by_team[away_id]
            home_key = (home_row["game_id"], home_id)
            away_key = (away_row["game_id"], away_id)

            if pin_completed and is_completed(home_row) and actual_team_win(home_row) is not None:
                home_result = actual_team_win(home_row)
                wins[home_key][wc] = home_result
                wins[away_key][wc] = "0" if home_result == "1" else "1"
                continue

            home_fpi = fpi_by_team.get(home_id, {}).get(sim_col)
            away_fpi = fpi_by_team.get(away_id, {}).get(sim_col)
            if home_fpi is None or away_fpi is None:
                continue

            margin = home_fpi - away_fpi
            p_home = win_probability(margin, 1.0)
            home_wins = rng.random() < p_home
            wins[home_key][wc] = "1" if home_wins else "0"
            wins[away_key][wc] = "0" if home_wins else "1"
...
def simulate_from_margins(
    ...
    *,
    ...
    fpi_by_team = _build_sim_fpi_lookup(rows, win_cols)
    flex_wins = _simulate_pac12_flex(
        flex_rows_by_team,
        margin_cols,
        win_cols,
        fpi_by_team,
...
def main() -> int:
    ...
    if margin_cols and not args.legacy_fpi:
        ...
        simulated = simulate_from_margins(
            ...
            rng,
            margin_cols,
            pin_completed=args.pin_completed,
            main_seed=args.seed,
        )
        win_cols = [margin_to_win_col(mc) for mc in margin_cols]
