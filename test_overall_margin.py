...
class OverallComponentWeightsTests(unittest.TestCase):
    ...
    def test_one_game_base_weights(self) -> None:
    ...
    def test_four_games_base_weights(self) -> None:
...
class OverallBlendWeightsTests(unittest.TestCase):
    def test_zero_games(self) -> None:
        self.assertEqual(overall_blend_weights(0), (1.0, 0.0, 0.0))

    def test_one_game(self) -> None:
        self.assertAlmostEqual(overall_blend_weights(1)[0], 0.85)
        self.assertAlmostEqual(overall_blend_weights(1)[1], 0.05)
        self.assertAlmostEqual(overall_blend_weights(1)[2], 0.10)

    def test_four_games(self) -> None:
        w_pre, w_some, w_algo = overall_blend_weights(4)
        self.assertAlmostEqual(w_pre, 0.49375)
        self.assertAlmostEqual(w_some, 0.16875)
        self.assertAlmostEqual(w_algo, 0.3375)

    def test_six_games(self) -> None:
        w_pre, w_some, w_algo = overall_blend_weights(6)
        self.assertAlmostEqual(w_pre, 0.0)
        self.assertAlmostEqual(w_some, 0.3102, places=3)
        self.assertAlmostEqual(w_algo, 0.6898, places=3)

    def test_seven_games(self) -> None:
        w_pre, w_some, w_algo = overall_blend_weights(7)
        self.assertAlmostEqual(w_pre, 0.0)
        self.assertAlmostEqual(w_some, 0.2152, places=3)
        self.assertAlmostEqual(w_algo, 0.7848, places=3)

    def test_eight_games(self) -> None:
        w_pre, w_some, w_algo = overall_blend_weights(8)
        self.assertAlmostEqual(w_pre, 0.0)
        self.assertAlmostEqual(w_some, 0.0729, places=3)
        self.assertAlmostEqual(w_algo, 0.9271, places=3)

    def test_nine_or_more_games(self) -> None:
        self.assertEqual(overall_blend_weights(9), (0.0, 0.0, 1.0))
        self.assertEqual(overall_blend_weights(15), (0.0, 0.0, 1.0))
...
class ComputeOverallMarginTests(unittest.TestCase):
    ...
    def test_one_game_blend(self) -> None:
        ...
        expected = 0.85 * 30.0 + 0.05 * 10.0 + 0.10 * 20.0
