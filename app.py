"""CFB Allen Ratings — in-season rankings and projections viewer."""
...
class DataStore:
    def __init__(self, data_dir: Path) -> None:
        ...
        self.rankings = self._load_optional(data_dir / "rankings.json") or {
            "current_week": 0,
            "weeks": [],
            "by_week": {},
        }
        self.records = self._load_optional(data_dir / "records.json") or {}
    ...
    def rankings_for_week(self, week: int) -> dict[str, dict]:
        return {
            row["team_id"]: row
            for row in self.rankings.get("by_week", {}).get(str(week), [])
        }
    ...
    def leaderboard_for_week(
        ...
        rows: list[dict] = []
        for lb_row in self.leaderboard:
            tid = lb_row.get("team_id")
            if not tid or not is_fbs_conference(lb_row.get("conference", "")):
                continue
            row = dict(lb_row)
        ...
        if ranking_mode == "algorithm":
            rows.sort(
                key=lambda r: (
                    r.get("algorithm_margin") is None,
                    -(r.get("algorithm_margin") or -999),
...
def create_app() -> Flask:
    ...
    def inject_globals():
        ...
        return {
            ...
            "game_url": game_url,
            "preseason_viewer_url": PRESEASON_VIEWER_URL,
            "team_search": [
    ...
    def leaderboard():
        ...
        try:
            week = int(week_raw) if week_raw else int(default_week)
        except ValueError:
            week = int(default_week)

        rows = store.leaderboard_for_week(week, ranking_mode=ranking_mode)
        if conf_filter:
    ...
    def team_detail(team_id: str):
        ...
        merged = {
            ...
            "fbs_games_played": lb.get("fbs_games_played", 0),
        }
